(:  NAME: DH_Class_Geo_Machine_Reading.xquery
    ...
    SUMMARY:  XQuery with two inputs: 1) TEI Collection (Syriaca.org Gazetteer), and; 2) Stanford CoreNLP Text (Chronicles of Edessa).
...
declare function local:match-place($places as xs:string*, $sentence as element()?)   as item()*
    {
    (:  1) Find placeNames from Geographic Data (Syriaca.org) in the Text (Chronicles of Edessa)
...
declare function local:create-places($geo) as item()*
    ...
    (: 201403017: AD- Input Matching Text Data: Chronicles of Edessa :)
    let $input := fn:doc("/db/test/input_Edessa.xml")
    
    let $TEI :=
    (: 201403021: AD- Iterate through each sentence in Matching Text Data:  Chronicles of Edessa :)
    ...
    <teiHeader>
        <fileDesc>
            <titleStmt>
        <title>The Chronicle of Edessa.  The Journal of Sacred Literature, New Series [=Series 4], vol. 5 (1864) pp. 28-45</title>
    </titleStmt>
        <publicationStmt>
            <publisher>Anthony Davis</publisher>
        </publicationStmt>
            <sourceDesc>
                <bibl>http://www.tertullian.org/fathers/chronicle_of_edessa.htm</bibl>
            </sourceDesc>
    </fileDesc>
    </teiHeader>
    <text>
